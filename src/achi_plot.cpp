...
#include <achi/phase1.hpp>
#include <achi/phase2.hpp>
#include <achi/phase3.hpp>
#include <achi/phase4.hpp>
#include <achi/util.hpp>
#include <achi/copy.h>
...
int main(int argc, char** argv)
{
	...
	cxxopts::Options options("achi_plot",
		"Multi-threaded pipelined Achi k32 plotter"
	...
	if(pool_key_str.empty()) {
		std::cout << "Pool Public Key (48 bytes) needs to be specified via -p <hex>, see `achi keys show`." << std::endl;
		return -2;
	}
	if(farmer_key_str.empty()) {
		std::cout << "Farmer Public Key (48 bytes) needs to be specified via -f <hex>, see `achi keys show`." << std::endl;
		return -2;
	}
	if(tmp_dir.empty()) {
	...
	if(pool_key.size() != bls::G1Element::SIZE) {
		std::cout << "Invalid poolkey: " << bls::Util::HexStr(pool_key) << ", '" << pool_key_str
			<< "' (needs to be " << bls::G1Element::SIZE << " bytes, see `achi keys show`)" << std::endl;
		return -2;
	}
	...
	{
		const std::string path = tmp_dir + ".achi_plot_tmp";
	...
	{
		const std::string path = tmp_dir2 + ".achi_plot_tmp2";
		if(auto file = fopen(path.c_str(), "wb")) {
			fclose(file);
			remove(path.c_str());
		} else {
	...
	{
		const std::string path = final_dir + ".achi_plot_final";
		if(auto file = fopen(path.c_str(), "wb")) {
			fclose(file);
			remove(path.c_str());
		} else {
	...
	
	{
		...
		for(int i = 0; i < num_files_max; ++i) {
			const std::string path = tmp_dir + ".achi_plot_tmp." + std::to_string(i);
			if(auto file = fopen(path.c_str(), "wb")) {
	...
	std::cout << "Multi-threaded pipelined Achi k32 plotter"; 
	#ifdef GIT_COMMIT_HASH
